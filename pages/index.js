...
function ECGCanvas({ data }) {
  ...
  return <canvas ref={ref} width={460} height={90} style={{ width:"100%", height:90, borderRadius:8 }} />;
}
...
export default function Dashboard() {
  ...
  const donutSegs = [
    ...
    { value: risk>0?Math.min(risk,40):0, color:"#f59e0b" },
    { value: risk>=60?risk-40:0, color:"#ef4444" },
  ];
  ...
  function PageContent() {
      ...
      <>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1.2fr 0.7fr 1fr",gap:16,marginBottom:16 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize:11,letterSpacing:2,opacity:0.7,marginBottom:8,textTransform:"uppercase",fontWeight:600 }}>Worker Health Status</div>
            <div style={{ fontSize:28,fontWeight:800,lineHeight:1.2,marginBottom:20 }}>
          ...
          <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)",
            ...
            <div style={{ position:"relative" }}>
              ...
              <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center" }}>
                <div style={{ fontSize:24,fontWeight:800,color:"#1e293b",fontFamily:"'DM Mono',monospace" }}>{risk}</div>
          ...
          <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)",
            ...
            <div style={{ fontSize:10,color:"#94a3b8",letterSpacing:1.5,textTransform:"uppercase",fontWeight:600,marginBottom:12 }}>Live Status</div>
            <AlertRow label="Fall Detection" value={d.fallDetected?"YES":"No"} color={d.fallDetected?"#ef4444":"#22c55e"} />
            <AlertRow label="4G Signal" value={`${d.signal??26}/31`} color="#6366f1" />
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:16 }}>
          {[
            {label:"Heart Rate",value:hr,unit:"bpm",color:"#ef4444",icon:"❤️",sub:hr>120?"⚠ HIGH":hr<50?"⚠ LOW":"Normal",mini:hrHist,delay:"0.2s"},
            {label:"SpO₂ Oxygen",value:spo2,unit:"%",color:"#3b82f6",icon:"💧",sub:spo2<90?"⚠ CRITICAL":"Normal",mini:spo2Hist,delay:"0.25s"},
            {label:"Temperature",value:temp,unit:"°C",color:"#f59e0b",icon:"🌡️",sub:temp>38.5?"⚠ FEVER":"Normal",mini:tempHist,delay:"0.3s"},
            {label:"ECG Signal",value:(hr/60).toFixed(2),unit:"Hz",color:"#8b5cf6",icon:"📡",sub:"PQRST waveform",mini:hrHist.map(v=>v/2),delay:"0.35s"},
          ].map((c,i)=>(
            <div key={i} style={{ animation:`fadeUp 0.4s ease ${c.delay} both` }}>
              <StatCard {...c} />
            </div>
          ))}
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1.5fr 1fr",gap:16 }}>
          ...
          <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)",
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:8,maxHeight:200,overflowY:"auto" }}>
              {alerts.length===0 && <div style={{ textAlign:"center",padding:"20px 0",color:"#94a3b8",fontSize:13 }}>All nominal</div>}
              {alerts.map((a,i)=>(
                <div key={i} style={{
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12,fontWeight:600,
                      color:a.type==="ok"?"#16a34a":a.type==="critical"?"#dc2626":"#d97706" }}>{a.msg}</div>
                    <div style={{ fontSize:10,color:"#94a3b8",marginTop:1 }}>{a.time} · HR:{a.hr} · SpO2:{a.spo2}%</div>
                  </div>
                </div>
              ))}
            </div>
    ...
    if (page===1) return (
      <div style={{ animation:"fadeUp 0.4s ease both" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16 }}>
          <StatCard label="Current HR" value={hr} unit="bpm" color="#ef4444" icon="❤️"
            sub={hr>120?"⚠ HIGH — above 120 bpm":hr<50?"⚠ LOW — below 50 bpm":"Normal range 60–100 bpm"} mini={hrHist} />
        ...
        <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
          <div style={{ fontWeight:700,fontSize:14,color:"#1e293b",marginBottom:14 }}>ECG Waveform</div>
          <ECGCanvas data={ecgBuf} />
        </div>
      </div>
    );
    ...
    if (page===2) return (
      <div style={{ animation:"fadeUp 0.4s ease both" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
          <StatCard label="SpO₂ Level" value={spo2} unit="%" color="#3b82f6" icon="💧"
            sub={spo2<90?"⚠ CRITICAL — below 90%":spo2<95?"Low — below 95%":"Normal above 95%"} mini={spo2Hist} />
          <div style={{ background:"white",borderRadius:16,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display:"flex",justifyContent:"space-between",marginTop:8,fontSize:11,color:"#94a3b8" }}>
              ...
              <span>Avg: {(spo2Hist.reduce((a,b)=>a+b,0)/spo2Hist.length).toFixed(1)}%</span>
            </div>
            <div style={{ marginTop:16,padding:"12px",borderRadius:10,background:spo2<90?"#fef2f2":"#f0fdf4",
                          border:`1px solid ${spo2<90?"#fecaca":"#bbf7d0"}`,fontSize:13,
                          color:spo2<90?"#dc2626":"#16a34a",fontWeight:600 }}>
              {spo2<90?"⚠ Emergency — low oxygen detected":"✓ Oxygen levels normal"}
            </div>
          </div>
        </div>
      </div>
    ...
    if (page===3) return (
      <div style={{ animation:"fadeUp 0.4s ease both" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16 }}>
          <StatCard label="Body Temperature" value={temp} unit="°C" color="#f59e0b" icon="🌡️"
            sub={temp>38.5?"⚠ FEVER — above 38.5°C":"Normal range 36.1–37.2°C"} mini={tempHist} />
          <div style={{ background:"white",borderRadius:16,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display:"flex",justifyContent:"space-between",marginTop:8,fontSize:11,color:"#94a3b8" }}>
              <span>Min: {Math.min(...tempHist).toFixed(2)}°C</span>
              <span>Max: {Math.max(...tempHist).toFixed(2)}°C</span>
            </div>
            <div style={{ marginTop:16,padding:"12px",borderRadius:10,background:temp>38.5?"#fef2f2":"#f0fdf4",
                          border:`1px solid ${temp>38.5?"#fecaca":"#bbf7d0"}`,fontSize:13,
                          color:temp>38.5?"#dc2626":"#16a34a",fontWeight:600 }}>
              {temp>38.5?"⚠ Fever detected — seek medical attention":"✓ Temperature normal"}
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16 }}>
          <div style={{ background:"white",borderRadius:20,padding:"24px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)",
            ...
            <div style={{ position:"relative" }}>
              ...
              <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center" }}>
                <div style={{ fontSize:32,fontWeight:800,color:"#1e293b",fontFamily:"'DM Mono',monospace" }}>{risk}</div>
                <div style={{ fontSize:12,color:"#94a3b8" }}>/100</div>
              </div>
            </div>
            <div style={{ marginTop:16,fontSize:14,fontWeight:700,
              color:risk<30?"#16a34a":risk<60?"#d97706":"#dc2626" }}>
              {risk<30?"LOW RISK":risk<60?"MODERATE RISK":"HIGH RISK"}
            </div>
          </div>
          <div style={{ background:"white",borderRadius:20,padding:"24px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
            <div style={{ fontSize:10,color:"#94a3b8",letterSpacing:1.5,textTransform:"uppercase",fontWeight:600,marginBottom:12 }}>Risk Breakdown</div>
            {[
              {label:"Fall Detected",score:d.fallDetected?40:0,max:40,color:"#ef4444"},
              {label:"Heart Rate",score:hr>120?25:hr<50?25:0,max:25,color:"#f97316"},
              {label:"SpO₂ Level",score:spo2<90?30:0,max:30,color:"#3b82f6"},
              {label:"Temperature",score:temp>38.5?20:0,max:20,color:"#f59e0b"},
            ].map((r,i)=>(
          ...
          <div style={{ background:"white",borderRadius:20,padding:"24px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ marginTop:16,padding:12,borderRadius:10,
              background:risk>=60?"#fef2f2":risk>=30?"#fefce8":"#f0fdf4",
              border:`1px solid ${risk>=60?"#fecaca":risk>=30?"#fde68a":"#bbf7d0"}`,
              fontSize:13,fontWeight:600,color:risk>=60?"#dc2626":risk>=30?"#d97706":"#16a34a" }}>
              {risk>=60?"⚠ Critical — immediate action needed":risk>=30?"⚠ Warning — monitor closely":"✓ Safe — all readings normal"}
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background:"white",borderRadius:20,padding:"24px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
          {alerts.length===0 && (
            <div style={{ textAlign:"center",padding:"48px 0",color:"#94a3b8",fontSize:15 }}>
              No alerts yet — system is nominal
            </div>
          )}
            ...
            {alerts.map((a,i)=>(
              <div key={i} style={{
                ...
                <div style={{ width:12,height:12,borderRadius:"50%",flexShrink:0,
                  background:a.type==="ok"?"#22c55e":a.type==="critical"?"#ef4444":"#f59e0b",
                  boxShadow:`0 0 8px ${a.type==="ok"?"#22c55e":a.type==="critical"?"#ef4444":"#f59e0b"}` }} />
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:14,fontWeight:700,color:"#1e293b" }}>{a.msg}</div>
                  <div style={{ fontSize:12,color:"#94a3b8",marginTop:3 }}>
                    {a.time} &nbsp;·&nbsp; Heart Rate: {a.hr} bpm &nbsp;·&nbsp; SpO₂: {a.spo2}%
                  </div>
                </div>
    ...
    // GPS page — real Google Maps
    if (page===6) return (
      <div style={{ animation:"fadeUp 0.4s ease both" }}>
        <div style={{ fontSize:20,fontWeight:800,color:"#1e293b",marginBottom:20 }}>GPS Location — SIM7600EI GNSS</div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 300px",gap:16 }}>
          {/* Real Google Maps embed */}
          <div style={{ background:"white",borderRadius:20,overflow:"hidden",boxShadow:"0 2px 16px rgba(0,0,0,0.06)",height:480 }}>
            <iframe
              title="GPS Map"
              width="100%"
              height="100%"
              ...
              loading="lazy"
              allowFullScreen
              src={`https://maps.google.com/maps?q=${(d.latitude||17.8687).toFixed(6)},${(d.longitude||78.2322).toFixed(6)}&z=16&output=embed&t=m`}
            />
          </div>
          {/* Coords panel */}
          <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
            <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ fontFamily:"'DM Mono',monospace",fontSize:13,color:"#1e293b",lineHeight:2 }}>
                <div>LAT: {(d.latitude||17.8687).toFixed(6)}°N</div>
                <div>LNG: {(d.longitude||78.2322).toFixed(6)}°E</div>
              </div>
            </div>
            <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ fontSize:13,color:"#475569",lineHeight:1.8 }}>
                <div style={{ fontWeight:600,color:"#1e293b" }}>BVRIT Campus</div>
                <div>Vishnupur, Narsapur</div>
                <div>Medak, Telangana</div>
                <div>India — 502313</div>
              </div>
            </div>
            <div style={{ background:"white",borderRadius:20,padding:"20px",boxShadow:"0 2px 16px rgba(0,0,0,0.06)" }}>
              <div style={{ fontSize:10,color:"#94a3b8",letterSpacing:1.5,textTransform:"uppercase",fontWeight:600,marginBottom:12 }}>GPS Module</div>
              <AlertRow label="Module" value="SIM7600EI" color="#6366f1" />
              <AlertRow label="Signal" value={`${d.signal??26}/31`} color="#22c55e" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{
          ...
          {PAGES.map((p,i)=>(
            <div key={i} title={p.label} onClick={()=>setPage(i)} style={{
              ...
              color:page===i?"white":"#94a3b8",marginBottom:6,
            }}>{p.icon}</div>
          ))}
        </div>
          ...
          <div style={{
            ...
            <div style={{ display:"flex",alignItems:"center",gap:12 }}>
              ...
              <div style={{
                ...
                <span style={{ width:7,height:7,borderRadius:"50%",display:"inline-block",
                  background:statusColor,animation:stat!=="NORMAL"?"pulse 0.6s infinite":"none" }} />
                {stat}
              </div>
            </div>
          </div>
